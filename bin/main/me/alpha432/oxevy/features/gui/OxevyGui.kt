...
class OxevyGui : Screen(Component.literal("Oxevy")) {
    ...
    private val accent: Color get() = Color(0, 212, 255) // Cyan accent
    ...
    private fun drawSidebar(ctx: GuiGraphics, gx: Float, gy: Float, mx: Int, my: Int) {
        ...
        val brand = "OX"
        ctx.drawString(mc.font, brand, gx.toInt() + (SIDEBAR_W - mc.font.width(brand)) / 2, gy.toInt() + 12, ACCENT_PRIMARY)
    ...
    private fun getComponentHeight(setting: Setting<*>): Int {
        val comp = getOrCreateComponent(setting, 0, 0, DRAWER_W - 24)
        ...
        if (showDrawer) {
            ...
            if (mx in drawerX..<gx + GUI_W && my in gy..<gy + GUI_H) {
                ...
                for (setting in mod.getSettings().filter { it.name != "Enabled" && it.name != "DisplayName" && it.isVisible }) {
                    ...
                    val comp = getOrCreateComponent(setting, drawerX + 12, sy, DRAWER_W - 24)
    ...
    companion object {
        const val SIDEBAR_W = 70
        const val GUI_W = 750
        const val GUI_H = 520
        const val CARD_H = 40
        const val CARD_GAP = 6
        const val DRAWER_W = 300
        const val SEARCH_H = 36
        const val TOPBAR_H = 36
        ...
        val ACCENT_PRIMARY = 0xFF00D4FF.toInt()
        val ACCENT_SECONDARY = 0xFF0099FF.toInt()
        val ACCENT_DIM = 0x1400D4FF.toInt()
        val ACCENT_HOVER = 0x2200D4FF.toInt()
