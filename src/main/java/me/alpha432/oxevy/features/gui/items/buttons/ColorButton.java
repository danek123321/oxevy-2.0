...
public class ColorButton extends Button {
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        drawString(this.getName(), this.x + 2.3f, this.y - 1.7f - (float) OxevyGui.getClickGui().getTextOffset(), textColor);
        ...
        if (open) {
            ...
            RenderUtil.rect(context, pickerX, this.y + yOffset, pickerX + buttonWidth, this.y + yOffset + 14,
                    hoveringCopy ? Oxevy.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha()) : 0x11555555);
            int textColorCopy = ColorUtil.toRGBA(ClickGuiModule.getInstance().color.getValue());
            drawString("Copy", pickerX + buttonWidth / 2.0 - mc.font.width("Copy") / 2.0, this.y + yOffset + 3, textColorCopy);
            ...
            RenderUtil.rect(context, pickerX + buttonWidth + 1, this.y + yOffset, pickerX + buttonWidth * 2 + 1, this.y + yOffset + 14,
                    hoveringPaste ? Oxevy.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha()) : 0x11555555);
            int textColorPaste = ColorUtil.toRGBA(ClickGuiModule.getInstance().color.getValue());
            drawString("Paste", pickerX + buttonWidth + buttonWidth / 2.0 - mc.font.width("Paste") / 2.0 + 1, this.y + yOffset + 3, textColorPaste);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0) {
            ...
            if (hoveringCopy) {
                OxevyGui.setColorClipboard(setting.getValue());
                mc.getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1f));
            }
            if (hoveringPaste && OxevyGui.getColorClipboard() != null) {
                setting.setValue(OxevyGui.getColorClipboard());
                hsb = Color.RGBtoHSB(setting.getValue().getRed(), setting.getValue().getGreen(), setting.getValue().getBlue(), null);
                mc.getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1f));
            }
        }
